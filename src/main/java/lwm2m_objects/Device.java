...
public class Device extends BaseInstanceEnabler {
  ...
  //  private static final Random RANDOM = new Random();
    ...
    private int getBatteryLevel() {
       // System.out.println(hal.getPowerSources()[0].getRemainingCapacityPercent());
        return (int)(hal.getPowerSources()[0].getRemainingCapacityPercent() * 100 );
    }
    ...
    private int getBatteryStatus() {
        PowerSource p = hal.getPowerSources()[0];
        double remainingLevel = p.getRemainingCapacityPercent()*100;
        if(p.isCharging()){
            return 1;
        } else if (remainingLevel > 95){
            return 2;

            // 3 is for if the battery is damaged
        } else if (remainingLevel < 20){
            return 4;
        }
